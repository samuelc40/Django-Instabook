...
#     return render(request, 'index.html', {'user_profile': user_profiles, 'posts': feed_list, 'suggestions_username_profile_list': suggestions_username_profile_list[:4]})
...
def search(request):
    ...
    if request.method == 'POST':
        ...
        username_profile_lists = list(chain(*username_profile_lists))
    ...
    if request.method == 'POST':
        ...
        if password == password2:
            if User.objects.filter(email=email).exists():
                ...
                messages.info(request, 'Username already does exists!')
                return redirect('signup')
            else:
                user = User.objects.create_user(username=username, email=email, password=password)
                user.save()

                # log user in and redirect to the settings
